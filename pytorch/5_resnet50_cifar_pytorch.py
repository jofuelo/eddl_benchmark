...
class ResnetBlock(nn.Module):
    def __init__(self, strides, nf, nf0, reps, bn):
        ...
        self.adapt_layer = nn.Conv2d(nf0, nf*4, kernel_size=1, stride=strides, padding=0, bias=False)
        for i in range(reps):
            if bn:
                self.layers.append(nn.Sequential(
                    nn.Conv2d(nf0, nf, kernel_size=1, stride=strides, padding=0, bias=False),
                    nn.BatchNorm2d(nf, eps=0.001, momentum=0.99),
                    nn.ReLU(),
                    nn.Conv2d(nf, nf, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.BatchNorm2d(nf, eps=0.001, momentum=0.99),
                    nn.ReLU(),
                    nn.Conv2d(nf, nf*4, kernel_size=1, stride=1, padding=0, bias=False),
                    nn.BatchNorm2d(nf*4, eps=0.001, momentum=0.99)))
            else:
                self.layers.append(nn.Sequential(
                    nn.Conv2d(nf0, nf, kernel_size=1, stride=strides, padding=0, bias=False),
                    nn.ReLU(),
...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(
            ResnetBlock(1, 64, 64, 3, bn),
            ResnetBlock(2, 128, 256, 4, bn),
            ResnetBlock(2, 256, 512, 6, bn),
            ResnetBlock(2, 512, 1024, 3, bn))
        
        self.fcout = nn.Linear(2048, 10)
