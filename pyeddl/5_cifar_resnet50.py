...
def resnet_block(l0, nf, bn, reps, downsample):
  for i in range(reps):
    ...
    l1 = eddl.GlorotUniform(eddl.Conv(l0, nf, [1,1], [stri,stri], "same", False))
    if(bn):
    ...
    l1 = eddl.GlorotUniform(eddl.Conv(l1, nf*4, [1,1], [1,1], "same", False))
    if(bn):
        l1 = eddl.BatchNormalization(l1, 0.99, 0.001, True, "")

    if(i==0):
      l0 = eddl.GlorotUniform(eddl.Conv(l0, nf*4, [1,1], [stri,stri], "same", False))
